...
structural data, and explicitly marked partial or experimental entries. The
...
implementation. Entries that provide only fusion rules or imported candidate
...
than tensor-equivalence classes. A candidate, fusion-rules-only, legacy, or
experimental entry supports only the operations stated on its detailed page.
...
| [Cocycle twists computed with GAP/HAP](@ref hap-cocycle-twists) | `twisted_graded_vector_spaces(K,G,i)` | Experimental helper; class enumeration and tuple/coefficient translations are backend-dependent |
| [Convolution categories](@ref convolution-models) | `convolution_category(K,X::GSet)` | Standard convolution bifunctor; experimental associator and duality implementation |
...
| [$E_6$ candidate data](@ref e6-data) | `E6subfactor()` | Rank 3, fusion multiplicity 2; associators do not satisfy the pentagon |
...
| [Haagerup $H_1$ artifact](@ref haagerup-h1) | `haagerup_H1()` | Published $H_1$ fusion ring with an exact artifact associator in a gauge not identified in the literature |
...
| [Haagerup legacy $H_2$ data](@ref haagerup-auxiliary-data) | `unitary_haagerup_H2()` | Legacy exact table with no identified published gauge |
...
| [Vercleyen–Slingerland candidate data](@ref vercleyen-slingerland-data) | `cat_fr_8122(n)`, `cat_fr_9143()` | Fusion rings and ancillary sources identified; imported $F$-basis ordering unverified |
